...
- [x] 6.6 UNBLOCKED as predicted — the flag now rides the generated scheme's test action (`environmentVariables` in project.yml), expanded from the build setting of the same name, so `xcodebuild ... CLAY_GOLDEN_REBASELINE=1` reaches an app-hosted unit bundle and an ordinary run expands to empty and stays in compare mode. `TEST_RUNNER_` is gone from the script; it only ever addressed UI-test runners
- [x] 6.7 Extraction verified end to end: 23 captures resolve to 20 reference images in `app/Tests/ClaySpaceTests/Goldens` (three brushes share a view). One change to the script was needed beyond 6.6 — it aborted under `set -e` whenever the capture run had a failing test, which meant no brush could be baselined until every brush passed. A red brush still renders, so failures are now reported and the extraction continues
- [x] 6.8 Simulator baselines committed for iPad16,4, captured against ClayCore v0.24.1 and re-verified unchanged at v0.24.2 — the goldens do not drift across that bump. Two carry known-broken output (`grab`, `fill`, both spec'd in fix-voxel-grab-and-fill) and MUST be re-baselined when that change lands; their geometric probes still fail loudly, so the images hide nothing. Device baselines still to capture — a simulator reference cannot stand in for an iPad's
