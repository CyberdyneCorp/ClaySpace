...
- [ ] 4.11 **BLOCKING: the regional volume swap tears the surface when relax rides it.** With a pronounced bump seeded and a relax region small against it, the probe line goes
  ...
  - Moved to its own change, `fix-regional-swap-tearing`, with the reproduction, the captures and the eliminations. Smooth stays blocked on it
- [ ] 4.12 **Still blocked after the bake fix, on a SECOND defect in the same area.** The holes are gone and the render now matches the document (document vs raycast agree to 0.01 at every probe, against 0.48 before). But the swap still moves the surface by ~0.16, asymmetrically, and **the effect is identical at relax strength 0.05 and 1.00** — so it is not the verb, it is the transport. `replaceRegion` is lossy over relief geometry in the app even though the standalone C reproduction shows the ABI's sampling and swap are faithful to 0.0014 for a single relief sphere
  - Most likely difference: the app's relief comes from STROKE CHAINS (many nodes via `beginStroke`), not one item with `CLAY_OP_RELIEF` as the C reproduction builds. The next step is to reproduce with a chain rather than a sphere
  - Smooth cannot be verified as smoothing while the transport changes the surface more than the verb does. Blocked, for a documented reason, on a defect that belongs to `fix-regional-swap-tearing`
  - **UNBLOCKED at ClayCore v0.25.0.** The transport defect was `ctape_volume` reading its blob offsets as absolute (upstream `ba803da`); with it fixed, all seven `RegionalSwapTests` pass and the guard that made these verbs refuse over strokes is gone. What the transport was hiding is now visible and is a DIFFERENT problem, recorded as 4.12b
  - `RegionalSwapTests` PASSES this at its 0.3 threshold, which was set to catch holes. It needs a tighter bound now that the holes are fixed `warpRadius * 1.3` covers the whole test ball, so the relax acts globally: every probe shifts ~0.065 together and the bump's relief is 0.0088 before, 0.0094 after — unchanged. The seeded bump is also too shallow (0.015 across the probe line). Needs a relax region small against the subject, and a pronounced bump
...
- [ ] 4.12b **Smooth is directionally right and effectively INERT.** With the transport fixed, the `smooth` matrix fixture still fails — but now for its own reason: it moves the peak by `2.5e-05` against the matrix's `minDelta` of `0.004`, about two orders of magnitude short. Attribution is measured, not inferred: at HEAD (guard in place) the same fixture reports `peak moved 0.0` — a flat refusal — and with the guard removed it reports `2.5e-05`, so the verb now acts and barely moves clay.
  - `SmoothBrushTests/testABumpIsSmoothedAway` PASSES on the identical gesture (both drive `brushSize 0.1` along `centerTap`), because it asserts only `peakAfter > peakBefore` with no magnitude floor. Any nonzero movement satisfies it. That test cannot fail for weakness and needs the same floor the matrix uses — a probe that passes on `2.5e-05` is not verifying the claim
  - Open question is the verb's parameters, not the transport: the fixture smooths at radius `0.1` over a bump seeded with `brushSize 0.8`, so the region may simply be too small against the feature. Establish whether it is tuning or the relax call before changing either
