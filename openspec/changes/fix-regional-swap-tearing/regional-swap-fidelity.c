...
static float probe_op(int op, const char *label) {
    ...
    // A lump on its top, with the op under test. Relief and incise are region
    // ops and take a rounding; add and subtract ignore it.
    float lump_r = 0.16f;
    clay_item *lump = clay_item_create(CLAY_PRIM_SPHERE, &lump_r, 1);
    float pos[3] = { 0.0f, ball_r, 0.0f };
    clay_item_set_position(lump, pos);
    clay_item_set_op(lump, op);
    clay_item_set_blend(lump, CLAY_BLEND_QUADRATIC, 0.06f);
    clay_item_set_rounding(lump, 0.14f);
    clay_node_id n1 = 0;
    if (clay_layer_add_item(doc, layer, lump, &n1) != CLAY_OK) { fail("add lump"); return -1; }
    clay_item_destroy(lump);
    ...
    // Rebuild the pristine document to get the BEFORE field.
    {
        ...
        clay_item *l2 = clay_item_create(CLAY_PRIM_SPHERE, &lump_r, 1);
        clay_item_set_position(l2, pos);
        clay_item_set_op(l2, op);
        clay_item_set_blend(l2, CLAY_BLEND_QUADRATIC, 0.06f);
        clay_item_set_rounding(l2, 0.14f);
        clay_node_id q1 = 0;
        clay_layer_add_item(pristine, pl, l2, &q1);
        clay_item_destroy(l2);
...
int main(void) {
    printf("regional swap (box-subtract + volume-add) fidelity, by op\n");
    int mj = 0, mn = 0, pt = 0;
